...
class Dependence(models.Model):
    ...
    def add_dependence(admin_id, sub_id):
        user_admin = Person.objects.get(id=admin_id)
        user_sub = Person.objects.get(id=sub_id)
        ...
        if user_sub.role == "":
            user_sub.role = "Subordinate"
        elif user_sub.role == "Admin":
            user_sub.role = "Subordinate/Admin"

        user_admin.save()
        user_sub.save()

        new_dependence, _ = Dependence.objects.get_or_create(
            user_id_admin=admin_id, user_id_sub=sub_id
        )
